...
//              String type = origDescriptors[i].getPropertyType().toString();
  ...
  public static void copyBeanNotNull2Bean(Object databean,Object tobean)throws Exception
  {
      ...
      for (int i = 0; i < origDescriptors.length; i++) {
          String name = origDescriptors[i].getName();
          String name_ = origDescriptors[i].getName();
          char[] cs = name_.toCharArray();
          cs[0]-=32;
          name_ = String.valueOf(cs);
          if ("class".equals(name)) {
              continue;
          }
          if (PropertyUtils.isReadable(databean, name) && (PropertyUtils.isWriteable(tobean, name) || PropertyUtils.isWriteable(tobean, name.toLowerCase())) || PropertyUtils.isWriteable(tobean,name_)) {
              Object value = PropertyUtils.getSimpleProperty(databean, name);
              if(value!=null){
                  if(PropertyUtils.isWriteable(tobean, name)){
                      getInstance().setSimpleProperty(tobean, name, value);
                  }
                  if(PropertyUtils.isWriteable(tobean, name.toLowerCase())){
                      getInstance().setSimpleProperty(tobean, name.toLowerCase(), value);
                  }
                  if(PropertyUtils.isWriteable(tobean, name_)){
                      getInstance().setSimpleProperty(tobean, name_, value);
                  }
              }
          }
      }
  ...
  public static Map<String,Object> copyBean2Map(Object bean){
      Map<String,Object> result = new HashMap<>();
      copyBean2Map(result,bean);
      return result;
  }
