...
function initDataGrid() {
	...
	$("#datagrid").datagrid({
		url : ctx + "/db/resScript/findPageList?groupId=" + groupId + "&sourceId=" + sourceId,
		width : '100%',
...
function initToolbar() {
	var toolbar = [ {
		...
		iconCls : 'icon-add',
		handler : function() {
			window.location.href = ctx + "/db/resScript/resScriptForm";
		}
	}, '-', {
		text : '删除脚本',
		iconCls : 'icon-cut',
...
function formatOper(val, row, index) { // val:值  row与此相对应的记录行 index：该行索引从0开始
	return '<a href="#" onclick="editScript(' + index + ')">修改</a>';
}
...
function editScript(index) {
	...
	if (row) {
		window.location.href=ctx + "/db/resScript/resScriptForm?id="+row.id;
	}
}

/* 删除样式 */
function deleteScript(rows) {
	var ids = "";
	...
	$.ajax({
		type : "post",
		url : ctx + "/db/resScript/delete?id=" + ids,
		dataType : 'json',
		cache : false,
