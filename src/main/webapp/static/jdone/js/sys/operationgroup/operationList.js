...
function initDataGrid() {
	$("#datagrid").datagrid({
		url : ctx + "/sys/operation/findPageList",
		width : '100%',
		...
		columns : [ [ 
			...
			{field : 'mark',title : '权限标识',align : 'center',width : '25%'},
			{field : 'isEnableRange',title : '是否设置范围',align : 'center',width : '15%',
			...
			{field : 'isEnableAuth',title : '是否权限控制',align : 'center',width : '15%',
				formatter:function(value,row,index){
	          		  var statusStr;
...
function initToolbar() {
	var toolbar = [  {
		...
		iconCls : 'icon-add',
		handler : function() {
			parent.$("#mainForm").attr("src","operationForm?id="+pid);
			window.location.href=ctx + "/sys/operation/operationForm?pid="+pid;
		}
	} ,{
		text : '删除',
		iconCls : 'icon-cut',
...
function editUser(index) {
	...
	if (row) {
		window.location.href=ctx + "/sys/operation/operationForm?id="+row.id;
		parent.$("#mainForm").attr("src","operationForm?id="+row.id);
	}
}
...
function deleteRegion(rows) {
	...
	$.ajax({
		type : "post",
		url : ctx + "/sys/operation/deleteoperationById?id=" + ids,
		dataType : 'json',
		cache : false,
