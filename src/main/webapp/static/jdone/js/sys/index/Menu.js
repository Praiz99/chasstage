...
Menu.prototype._createContent = function($this) {
	var id = $this.attr('data-id'), name = $this.text(), href = $this
			.attr('href');
	var $tab = null;
	if ($this.is(this.defaultSelect)) {
		$tab = $('<li data-id="'
				+ id
				+ '" data-default="default"><a href="javascript:;" class="ue-clear"><span>'
				+ name + '</span></a></li>');
	} else {
		$tab = $('<li data-id="' + id
				+ '"><a href="javascript:;" class="ue-clear"><span>' + name
				+ '</span><i class="close-tab"></i></a></li>');
	}
	var $iframe = $('<iframe data-id="' + id
			+ '" width="100%" height="100%" frameborder="0" src="' + href
			+ '"></iframe>');
...
Menu.prototype._show = function($this) {
	var id = $this.attr('data-id'), $tabli = this.tab.find('li[data-id="' + id
			+ '"]');
	this.nav.find('li').removeClass('current');
	this.nav.find('li[data-id="' + id + '"]').addClass('current').parents(
			'.nav-li').siblings().find(".subnav").hide().end().end().addClass(
			'current').find(".subnav").show();
	if(this.nav.find('li[data-id="' + id + '"]').hasClass("lastsubnav-li")){
		this.nav.find('li[data-id="' + id + '"]').parent().click();
	}
...
Menu.prototype._close = function($this, flag) {
	// 判断是否是当前窗口
	var id = $this.attr('data-id'), current = this.cacheOpen[id], index = $this
			.index(), $tab = this.tab.find('li').eq(index - 1);
	// 执行关闭
	current.tab.remove();
	current.iframe.remove();
	current.more && current.more.remove();
	delete this.cacheOpen[id];
	if (current && current.nav.hasClass('current')) { // modify 2014-3-24
		this._show($tab);
	}
...
Menu.prototype._bindEvent = function() {
	...
	this.nav.on('click', 'li', function() {
		...
		$(this).addClass('current').siblings().removeClass("current").find(
				".subnav").hide().end().end().find(".subnav").show();
	});
	...
	this.nav.on('click', '.lastsubnav-li', function() {
		self._select($(this));
	});
	...
	if (!-[ 1, ]) {
		$('.more-bab-list').on(
				...
				'li',
				function() {
					$('.more-bab-list').find('.opt-panel-ml').css('bottom', 0)
							.css('bottom', 5);
					$('.more-bab-list').find('.opt-panel-mr').css('bottom', 0)
							.css('bottom', 5);
				});
	}
	...
	this.tab.next('.tab-more').click(
			function() {
				$('.more-bab-list')
						.css(
								'left',
								$(this).offset().left
										- $('.more-bab-list').width() / 2)
						.show();
				$(this).toggleClass('active');
			});
	/* window resize 事件 */
	resize(function(e) {
		var width = self._checkCanTab(), tempWinWindow = self.winWidth
				- $(window).width(), $tabMore = self.tab.next('.tab-more');
		$('.more-bab-list').hide();
		...
		if (width > 0 && tempWinWindow > 0) {
			...
			if ((width < -$last.width() - 130) && tempWinWindow < 0
					&& $last.size() == 1) {
...
Menu.prototype._select = function($this) {
	var id = $this.attr('data-id'), opened = this.cacheOpen[id];
	// 是否已经存在
	if (opened) {
		this._show($this);
	} else {
...
Menu.prototype._checkWidth = function(flag) {
	var size = this.tab.find('li').size(), intoMore = this.tab.find('li').eq(
			size - 2), intoId = intoMore.attr('data-id'), $tabMore = this.tab
			.next('.tab-more');
...
Menu.prototype._move2MorePanel = function(intoId) {
	...
	obj.tab.detach();
	var $more = $('<li data-id="' + intoId
			+ '"><span href="javascript:;"><i></i>' + name + '</span></li>');
...
Menu.prototype._move2tab = function(id, flag) {
	...
	var size = this.tab.find('li').size(), intoId = this.tab.find('li').eq(
			size - 1).attr('data-id'), tabLi = this.cacheOpen[id].tab, moreLI = this.cacheOpen[id].more;
