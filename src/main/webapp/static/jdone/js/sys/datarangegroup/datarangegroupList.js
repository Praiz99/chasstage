...
function doSearch(){
	...
	$("#menuTree").html("");
	setting.async.url = 'getDatarangeTree';
...
function clickTree(event, treeId, treeNode){
	...
	var url;
	if(treeNode.type==1){
		url = 'datarangeList?groupLevel='+treeNode.mark+"&pid="+treeNode.id;
	}else{
		url = 'datarangeForm?id='+treeNode.id;
	}
...
function UpdateTree(obj){
    ...
    var url;
		url = 'datarangegroupForm?id='+treenode.id;
	$("#mainForm").attr("src",url);
	}
}
//打开新增菜单页面
...
function openMenuSettings(obj){
	...
	if(obj == "parent"){
		...
		if(treenode.type==0){
			...
			var url ='datarangegroupForm?pid='+treenode.id;
			pid = treenode.id;
			$("#mainForm").attr("src",url);
			var sortarr = getMenuMaxSort(ztree.getNodes());
			desort =  Math.max.apply(null, sortarr)+1;
...
function saveMenuSettings(){
	...
	if($("#mainForm").attr("src").indexOf("datarangeList")<0){
		$.ajax({
			url:'saveDatarange',
