    ...
    <div class="yd-header">
        <table id="headTable" style="width: 100%">
            <tr>
                ...
                <th style="width: 40%;"><h1>审讯区人员情况</h1></th>
    ...
    <div id="A1" class="yd-container" style="height: 90%">
        <table style="width: 100%;height: 58px;">
            <tr style="background: #DCE3EA;">
                ...
                <td style='width: 20%;height: 16%;text-align: center'>分配审讯室</td>
            </tr>
        </table>
...
<script type="text/javascript">
    ...
    function getDpData(){
        ...
        $.ajax({
            type: "post",
            url: ctx + "/loginSxsDp/getSxsryList?baqid="+baqid,
            async:false,
            dataType: 'json',
            cache: false,
            success: function(data) {
                if(data.code == 0){
                    var str = "";
                    var str2 = "";
                    var sxsryList = dataFormat(data.sxsryList);
                    var sxsryZdList = dataFormat(data.sxsryZdList);
                    var zdCount = sxsryZdList.length;
                    for(j = 0,len=sxsryList.length; j < len; j++) {
                        str+="<tr>"+
                            "<td style='width: 10%;height: 16%;text-align: center'>"+ (zdCount+1) +"</td>"+
                            "<td style='width: 12%;height: 16%;text-align: center'>"+ (sxsryList[j].ryxm ? sxsryList[j].ryxm : "") +"</td>"+
                            "<td style='width: 12%;height: 16%;text-align: center'>"+ (sxsryList[j].ryxbName ? sxsryList[j].ryxbName : "") +"</td>"+
                            "<td style='width: 20%;height: 16%;text-align: center;overflow: hidden; white-space: nowrap;'><div class='scrollable-content'>"+ (sxsryList[j].rssj ? sxsryList[j].rssj : "")+"</div></td>"+
                            "<td style='width: 26%;height: 16%;text-align: center'>"+ (sxsryList[j].dqwz ? sxsryList[j].dqwz : "") +"</td>"+
                            "<td style='width: 20%;height: 16%;text-align: center'>"+(sxsryList[j].qymc ? sxsryList[j].qymc : "") +"</td>"+
                            "</tr>";
                        zdCount++;
                    }

                    for(j = 0,len=sxsryZdList.length; j < len; j++) {
                        str2+="<tr style='height: 105px;background-color: #63E6E2'>"+
                            "<td style='width: 10%;height: 16%;text-align: center'>"+(j+1)+"</td>"+
                            "<td style='width: 12%;height: 16%;text-align: center'>"+ (sxsryZdList[j].ryxm ? sxsryZdList[j].ryxm : "") +"</td>"+
                            "<td style='width: 12%;height: 16%;text-align: center'>"+ (sxsryZdList[j].ryxbName ? sxsryZdList[j].ryxbName : "") +"</td>"+
                            "<td style='width: 20%;height: 16%;text-align: center;overflow: hidden; white-space: nowrap;'><div class='scrollable-content'>"+ (sxsryZdList[j].rssj ? sxsryZdList[j].rssj : "")+"</div></td>"+
                            "<td style='width: 26%;height: 16%;text-align: center'>"+ (sxsryZdList[j].dqwz ? sxsryZdList[j].dqwz : "") +"</td>"+
                            "<td style='width: 20%;height: 16%;text-align: center'>"+(sxsryZdList[j].qymc ? sxsryZdList[j].qymc : "") +"</td>"+
                            "</tr>";
                    }
                    ...
                    l2.innerHTML = str2;
                    var index = sxsryList.length + sxsryZdList.length;
                    o = index;
...
<script type="text/javascript">
    ...
    function onMessage(evt) {
        ...
        if("haiNingCorridorLocationInterrogation"==evt.data){
            getDpData()
        }
    }
