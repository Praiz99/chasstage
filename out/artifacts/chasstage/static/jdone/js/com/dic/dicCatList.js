...
function initToolbar() {
	var toolbar = [ {
		...
		iconCls : 'icon-add',
		handler : function() {
			window.location.href = ctx + "/com/dic/dicCatForm";
		}
	}, '-', {
		text : '删除',
		iconCls : 'icon-cut',
...
function formatOper(val, row, index) { // val:值  row与此相对应的记录行 index：该行索引从0开始
	return '<a href="#" onclick="editDicCat(' + index + ')">修改</a>';
}

function editDicCat(index) {
	$('#datagrid').datagrid('selectRow', index);// 选择一行，行索引从0开始。
	var row = $('#datagrid').datagrid('getRows')[index];
	if (row) {
		window.location.href=ctx + "/com/dic/dicCatForm?id="+row.id;
	}
}

/* 删除分类信息 */
function deleteDicCats(rows) {
	var ids = "";
	...
	$.ajax({
		type : "post",
		url : ctx + "/com/dic/deleteCats?ids=" + ids,
		dataType : 'json',
		cache : false,
