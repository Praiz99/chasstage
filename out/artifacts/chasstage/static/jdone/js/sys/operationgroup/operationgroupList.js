...
function doSearch(){
	...
	$("#menuTree").html("");
	setting.async.url = 'getOperationTree';
...
function clickTree(event, treeId, treeNode){
	...
	var url;
	if(treeNode.type==1){
		url = 'operationList?groupLevel='+treeNode.mark+"&pid="+treeNode.id;
	}else{
		url = 'operationForm?id='+treeNode.id;
	}
...
function UpdateTree(obj){
    ...
    var url;
		url = 'operationgroupForm?id='+treenode.id;
	$("#mainForm").attr("src",url);
	}
}
//打开新增菜单页面
...
function openMenuSettings(obj){
	...
	if(obj == "parent"){
		...
		if(treenode.type==0){
			...
			var url ='operationgroupForm?pid='+treenode.id;
			pid = treenode.id;
			$("#mainForm").attr("src",url);
			var sortarr = getMenuMaxSort(ztree.getNodes());
			desort =  Math.max.apply(null, sortarr)+1;
...
function openApiSettings(obj){
	...
	if(obj == "parent"){
		...
		if(treenode.type==0){
			$.messager.alert('提示','此为功能，不可增加功能!');
			return;
		}else{
			var url = 'operationForm';
			pid = treenode.id;
			$("#mainForm").attr("src",url);
			var sortarr = getMenuMaxSort(ztree.getNodes());
			desort =  Math.max.apply(null, sortarr)+1;
...
function saveMenuSettings(){
	...
	if($("#mainForm").attr("src").indexOf("operationList")<0){
		$.ajax({
			url:'saveOperation',
