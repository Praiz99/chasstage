...
function uploadify_onSelectError(file, errorCode, errorMsg){
	...
	case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
		if (settings.queueSizeLimit > errorMsg) {
			this.queueData.errorMsg = "已选文件的数量超过了上传文件数量限制(" + settings.uploadLimit + ")";
			
		} else {
			this.queueData.errorMsg = '所选文件的总大小超过了上传总大小限制(' + settings.queueSizeLimit + ')';
		}
		break;
	case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
...
function uploadify_onUploadError(file, errorCode, errorMsg) {
	...
	case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
		...
		break;
	case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
		errorString += '超过文件上传个数，最多每次上传'+settings.uploadLimit + '个文件';
		break;
	case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
