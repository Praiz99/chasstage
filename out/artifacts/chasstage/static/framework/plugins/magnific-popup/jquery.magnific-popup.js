...
MagnificPopup.prototype = {
	...
	addGroup: function(el, options) {
		...
		if(options.items) {
			options.isObj = true;
			el.unbind(eName).bind(eName, eHandler);
		} else {
			options.isObj = false;
			if(options.delegate) {
				el.unbind(eName).delegate(options.delegate,eName,eHandler);
			} else {
				options.items = el;
				el.unbind(eName).bind(eName, eHandler);
			}
	...
	proto: {
		initImage: function() {
			...
			_mfpOn(CLOSE_EVENT+ns, function() {
				...
				_window.unbind('resize' + EVENT_NS);
			});
		...
		initGallery: function() {
			...
			_mfpOn(OPEN_EVENT+ns, function() {
				...
				if(gSt.navigateByImgClick) {
					mfp.wrap.delegate('.mfp-img','click'+ns,function() {
...
(function() {
	var ghostClickDelay = 1000,
		...
		unbindTouchMove = function() {
			_window.unbind('touchmove'+ns+' touchend'+ns);
		},
		eName = 'mfpFastClick',
		ns = '.'+eName;
	...
	$.fn.destroyMfpFastClick = function() {
		$(this).unbind('touchstart' + ns + ' click' + ns);
		if(supportsTouch) _window.unbind('touchmove'+ns+' touchend'+ns);
	};
})();
